...
elseif (count($ex) == 2)
	$year = $ex[1];
$result2 = mysqli_query($conn, "select day, start, end from professionalhours where professionalEmail = '" . mysqli_real_escape_string($conn, $_SESSION["email"]) . "'");
$result3 = mysqli_query($conn, "select day, start, end from professionaloff where professionalEmail = '" . mysqli_real_escape_string($conn, $_SESSION["email"]) . "' and year='$year' and month='$month'");
$normalhours = array();
$closed = array();
if (mysqli_num_rows($result2) > 0)
{
	while($row = mysqli_fetch_assoc($result2))
	{
		...
		{
			$normalhours[$row["day"]] = array($row["start"],$row["end"]);
		}
	}
}
else
...
if (mysqli_num_rows($result3) > 0)
{
	while($row = mysqli_fetch_assoc($result3))
	{
		if (!is_null($row["day"]) && !array_key_exists($row["day"],$closed))
			$closed[$row["day"]] = array($row["start"],$row["end"]);
	}
}
...
month -= 1;
 $(function() {
	...
	?> &nbsp; <input type="submit" value="Change Date" /> <span id="datedetail"></span><br>
	<a href="addBreak.php">Change Lunch Break</a>
</form></div><hr>
...
{
	// $contents is the days in the current week
	foreach ($contents as $day)
	{
		...
		if ($day != "")
		{
			...
			if (!isPast($curdate))
			{
				...
				if (array_key_exists($day,$closed)) // If the professional is currently off on the current day
				{
					echo "<input type=\"button\" onclick=\"editDate(this)\" id=\"editbutton\" value=\"Edit\" /><br><div class='greyout' id='dayinfo'><input type=\"checkbox\" value=\"yes\" name=\"closed\" id=\"closed$day\" ";
					if ($closed[$day][0] == null)
						echo "checked ";
					echo "/><label for=\"closed$day\">Off All Day</label><br>Open: <input type=\"text\" id=\"open$day\" name=\"openhour\" maxlength=\"8\" size=\"8\" />
				...
				else
				{
					...
					{
						echo "<input type=\"button\" onclick=\"editDate(this)\" id=\"editbutton\" value=\"Edit\" /><br><div class='greyout' id='dayinfo'><input type=\"checkbox\" value=\"yes\" name=\"closed\" id=\"closed$day\" /><label for=\"closed$day\">Off All Day</label><br>Open: <input type=\"text\" id=\"open$day\" name=\"openhour\" maxlength=\"8\" size=\"8\" />
						<br>Close: <input type=\"text\" id=\"close$day\" name=\"closehour\" maxlength=\"8\" size=\"8\" /><br><input type=\"submit\" value=\"Change\" /><script>
						days.push('$day');
...
function validate(f)
{
	$.ajax({
			...
			success: function(xml) {
				...
				var valid = true;
				var d = new Date(year, month, day.val());
				$(xml).find('day').each(function(){
					if (parseInt($(this).attr('daynum')) == d.getDay())
					{
						if (!$(closedallday).is(':checked'))
						{
							...
							if (date1 != false && date2 != false)
							{
								...
								var date3 = new Date(2019, 03, 02, parseInt($(this).attr('openhour')), parseInt($(this).attr('openmin')));
								var date4 = new Date(2019, 03, 02, parseInt($(this).attr('closehour')), parseInt($(this).attr('closemin')));
								if (date1 < date3)
								{
									openhour.addClass("invalid");
									valid = false;
			...
			success: function(xml) {
				for (var i = 0; i < days.length; i++)
				{
					...
					$(xml).find('day').each(function(){
						if (parseInt($(this).attr('daynum')) == d.getDay())
						{
							var open = leading_zero($(this).attr('openhour')) + ":" + leading_zero($(this).attr('openmin'));
							var close = leading_zero($(this).attr('closehour')) + ":" + leading_zero($(this).attr('closemin'));
							$('#open' + day).timepicker({
							'step': 15,
							'minTime': open,
							'maxTime': close,
