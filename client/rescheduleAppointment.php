...
if (isset($_POST["date"]) && ((!isset($_SESSION["scheduling"]) && isset($_POST["id"])) || isset($_SESSION["scheduling"])) && $conn)
{
	...
	$_SESSION["day"] = $day;
	$query1 = mysqli_query($conn, "select barbershop.barbershopId, serviceId, name from professional inner join barbershop on professional.barbershopId = barbershop.barbershopId inner join scheduling on professional.email = scheduling.professionalEmail where md5(scheduling.id) = '" . mysqli_real_escape_string($conn, $_POST["id"]) . "'");
...
<form method="post" id="reschedule" onsubmit="return continuetoMakeAppointment(this);"><fieldset><legend>Change Day</legend>
...
while ($curFinish <= $close2)
{
	...
	{
		echo "<div><span class=\"reviewLink\" onclick=\"revealAppointment(this)\">" . $curTime->format("h:i A") . "</span></span><div class=\"reviewContents\" name=\"contents\" style=\"text-align:left;display:none;\"><form method=\"post\" action=\"reschedule.php\" /><input type=\"hidden\" name=\"time\" value=\"" . $curTime->format("h:i A") . "\" /><ul>";
		...
		echo "</ul><input type=\"submit\" value=\"Reschedule Appointment!\" name=\"book\" disabled /></form></div></div>";
	}
	$curTime = $curFinish;
